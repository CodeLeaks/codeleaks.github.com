...
对于这样的请求， twitter.com 会认出：哦，原来是你啊，欢迎回来。你要的东西在这里，你还剩下多少次 API 访问。


``` 
{'status': '200 OK', 'x-ratelimit-remaining': '329', ...

```


Twitter for iPhone 的第一宗 "罪" 即为在调用一些非帐号相关（即该请求无论由谁调用的结果都相同，不需要验证身份）的API时，会选择上文提到的第一种方法：API匿名调用。 Loren 这样的做法有它的一定道理，这些到底是一些无状态的 API，匿名是没有错的。 但是做出如此决定作者显然没有考虑很多用户在同一 IP 后的状况，如果有多个人在同一个出口 IP 访问 Twitter 的话，大家加起来访问这些 API 小时超过 150次 即为被网站无情地拒绝 :(
...
 
<h3>较慢的载入速度</h3>
载入较慢的第一原因即为默认载入的 Timeline 数量有点过多，像 Echofon 这样异步离线加载过早的推是个提升启动速度的好办法。
...
我们在这里自己看一下 Twitter for iPhone 显示一条推详细信息的界面。很不错是吧，有图片的话会显示嵌入的图片，如果推带有 Geocode 会显示嵌入的小地图，一切都显得非常和谐。
...
至此答案就清楚了：作者 "偷懒" 地在这里使用了 Webview 来实现某条推详细信息的显示。如果说这种实现方法有什么缺点的话，就是一个字，慢。 作者在这里加入时间很长的翻动效果主要就是为了掩饰以后再切换显示的 html 页面的内容的方式给人以不一致的体验。在其它一些客户端，比如 echofon 里就没有该问题，翻动上下推都是瞬间的事情。
<h3>薄弱的 Twitter List 支持</h3>
善用List的人几乎会把它当作第二个 Timeline, Twitter for iPhone的List实现显然已经不能满足List重度用户的胃口。问题有两个：1. 没有List内容的本地缓存，每次都要去下载 2. 调用切换不便。相信贴上两张图就高下立分，上图为 TweetList，下图为 Twitter for iPhone。
<img style="display:block;margin-left:auto;margin-right:auto;" title="tweet_list_list.jpg" src="http://codeleaks.files.wordpress.com/2010/08/tweet_list_list1.jpg" border="0" alt="tweet_list_list" width="320" height="480"><img style="display:block;margin-left:auto;margin-right:auto;" title="twitter_list.jpg" src="http://codeleaks.files.wordpress.com/2010/08/twitter_list1.jpg" border="0" alt="4897835628_445bdec89e-1.jpg" width="320" height="480">

<h3>开发者严重不给力</h3>
这点我就不多说了，心里还是希望Loren在加盟Twitter拿了不少股权以后，无论花了多少时间，能够拿出个像样的 Twitter for iPad 和 Tweetie for Mac 2. 其实如果从 App Store 整体角度来说，Twitter for iPhone 依旧是最好的免费客户端，且没有之一。我在这里说闲话也是纯属站着说话不腰疼，就这样吧。
